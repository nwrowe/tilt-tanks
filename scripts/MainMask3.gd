...
const WORLD_WIDTH_MIN: int = 1200
const WORLD_WIDTH_MAX: int = 3000
const WORLD_HEIGHT: int = 620
const TERRAIN_STEP: int = 3
const TERRAIN_MIN_Y: float = 230.0
const TERRAIN_MAX_Y: float = 505.0
const TANK_RADIUS: float = 16.0
const TANK_HALF_WIDTH: float = 24.0
const TANK_START_LEFT_X: float = 130.0
const TANK_EDGE_MARGIN: float = 130.0
const TANK_MOVE_SPEED: float = 58.0
const TANK_GROUND_SEARCH_UP: float = 42.0
const TANK_GROUND_SEARCH_DOWN: float = 70.0
const CANNON_LENGTH: float = 48.0
const PROJECTILE_RADIUS: float = 5.0
const EXPLOSION_RADIUS: float = 62.0
const DAMAGE_RADIUS: float = EXPLOSION_RADIUS * 1.15
...
const EDGE_WIDTH: float = 2.8

var rng: RandomNumberGenerator = RandomNumberGenerator.new()
var world_width: int = 1500
var terrain_cols: int = 0
...
func _build_overlay_ui() -> void:
	...
	mobile_left_button.button_down.connect(_on_left_down)
	mobile_left_button.button_up.connect(_on_left_up)
	mobile_right_button.button_down.connect(_on_right_down)
	mobile_right_button.button_up.connect(_on_right_up)
	mobile_fire_button.pressed.connect(_on_mobile_fire_pressed)
...
func _generate_terrain_mask() -> void:
	world_width = int(round(rng.randf_range(float(WORLD_WIDTH_MIN), float(WORLD_WIDTH_MAX))))
	terrain_cols = int(ceil(float(world_width) / float(TERRAIN_STEP))) + 1
	terrain_rows = int(ceil(float(WORLD_HEIGHT) / float(TERRAIN_STEP))) + 1
	solid.clear()
	for col: int in range(terrain_cols):
		var column: PackedByteArray = PackedByteArray()
		column.resize(terrain_rows)
		column.fill(0)
		solid.append(column)

	var surface_y_by_col: PackedFloat32Array = _make_surface_profile()
	for col: int in range(terrain_cols):
		var surface_y: float = surface_y_by_col[col]
		var start_row: int = clampi(int(floor(surface_y / TERRAIN_STEP)), 0, terrain_rows - 1)
		for row: int in range(start_row, terrain_rows):
			solid[col][row] = 1
...
func _make_surface_profile() -> PackedFloat32Array:
	var result: PackedFloat32Array = PackedFloat32Array()
	result.resize(terrain_cols)
	var control_spacing: float = rng.randf_range(75.0, 150.0)
	var control_points: Array[Vector2] = []
	var control_count: int = int(ceil(float(world_width) / control_spacing)) + 2
	var previous_y: float = rng.randf_range(360.0, 455.0)
	for i: int in range(control_count):
		var x: float = float(i) * control_spacing
		var y: float = clampf(previous_y + rng.randf_range(-78.0, 78.0), TERRAIN_MIN_Y, TERRAIN_MAX_Y)
		if x < 210.0 or x > float(world_width) - 210.0:
	...
	for col: int in range(terrain_cols):
		...
		if x > 230.0 and x < float(world_width) - 230.0:
			y += 4.0 * sin(x * 0.022 + rng.randf_range(-0.03, 0.03))
		result[col] = clampf(y, TERRAIN_MIN_Y, TERRAIN_MAX_Y)

	_flatten_profile_spawn(result, TANK_START_LEFT_X, 48.0)
	_flatten_profile_spawn(result, float(world_width) - TANK_EDGE_MARGIN, 48.0)
	return result

func _flatten_profile_spawn(profile: PackedFloat32Array, center_x: float, half_width: float) -> void:
	var sum: float = 0.0
	var count: int = 0
	for col: int in range(profile.size()):
		var x: float = float(col * TERRAIN_STEP)
		if absf(x - center_x) <= half_width:
			sum += profile[col]
			count += 1
	if count <= 0:
		return
	var flat_y: float = sum / float(count)
	for col: int in range(profile.size()):
		var x: float = float(col * TERRAIN_STEP)
		var dist: float = absf(x - center_x)
		if dist <= half_width:
			profile[col] = flat_y
		elif dist <= half_width + 40.0:
			profile[col] = lerpf(flat_y, profile[col], (dist - half_width) / 40.0)

func _carve_lake_basin() -> void:
	lake_width = rng.randf_range(LAKE_MIN_WIDTH, minf(LAKE_MAX_WIDTH, float(world_width) * 0.28))
	lake_x = rng.randf_range(280.0, maxf(290.0, float(world_width) - 280.0 - lake_width))
	lake_y = rng.randf_range(415.0, 455.0)
	var lake_center: float = lake_x + lake_width * 0.5
	var radius_x: float = lake_width * 0.65
	var water_bottom: float = lake_y + rng.randf_range(30.0, 58.0)
	for col: int in range(terrain_cols):
		var x: float = float(col * TERRAIN_STEP)
		var dist: float = absf(x - lake_center)
		if dist <= radius_x:
			var nx: float = dist / radius_x
			var basin_bottom: float = water_bottom + 20.0 * (1.0 - nx * nx)
			var top_row: int = clampi(int(floor(lake_y / TERRAIN_STEP)), 0, terrain_rows - 1)
			var bottom_row: int = clampi(int(ceil(basin_bottom / TERRAIN_STEP)), 0, terrain_rows - 1)
...
func _is_solid_cell(col: int, row: int) -> bool:
	if col < 0 or col >= terrain_cols or row < 0 or row >= terrain_rows:
		return false
	return solid[col][row] == 1

func _is_solid_at(pos: Vector2) -> bool:
	var col: int = int(round(pos.x / TERRAIN_STEP))
	var row: int = int(round(pos.y / TERRAIN_STEP))
	return _is_solid_cell(col, row)

func _find_surface_y(x: float) -> float:
...
func _find_local_surface_y(x: float, current_tank_y: float) -> float:
	...
	var start_row: int = clampi(int(floor((tank_feet_y - TANK_GROUND_SEARCH_UP) / TERRAIN_STEP)), 0, terrain_rows - 1)
	var end_row: int = clampi(int(ceil((tank_feet_y + TANK_GROUND_SEARCH_DOWN) / TERRAIN_STEP)), 0, terrain_rows - 1)
	for row: int in range(start_row, end_row + 1):
		if solid[col][row] == 1:
			return float(row * TERRAIN_STEP)
	return _find_surface_y(x)
...
func _settle_tanks_on_terrain() -> void:
	for player: int in range(2):
		tank_positions[player].y = _find_tank_surface_y(tank_positions[player].x, tank_positions[player].y) - TANK_RADIUS

func _is_in_lake(pos: Vector2) -> bool:
	if not lake_enabled:
		return false
	if pos.x < lake_x or pos.x > lake_x + lake_width:
		return false
	return pos.y >= lake_y and not _is_solid_at(pos)
...
func _update_tank_movement(delta: float) -> void:
	...
	if direction == 0.0:
		return
	var new_x: float = clampf(tank_positions[current_player].x + direction * TANK_MOVE_SPEED * delta, 45.0, float(world_width) - 45.0)
	...
	tank_positions[current_player].y = _find_tank_surface_y(new_x, tank_positions[current_player].y) - TANK_RADIUS
...
func _update_projectile(delta: float) -> void:
	...
	if _is_in_lake(projectile_pos):
		...
		return
	if _is_solid_at(projectile_pos):
		_explode(projectile_pos, true)
		return
	if projectile_pos.x < -100.0 or projectile_pos.x > float(world_width) + 100.0 or projectile_pos.y > float(WORLD_HEIGHT + 100):
		_explode(projectile_pos, false)
...
func _explode(pos: Vector2, carve_terrain: bool = true) -> void:
	...
	if carve_terrain:
		_erase_circle(pos, DAMAGE_RADIUS)
...
func reset_match() -> void:
	...
	mobile_left_pressed = false
	mobile_right_pressed = false
	_generate_terrain_mask()
	tank_positions = [Vector2(TANK_START_LEFT_X, 0.0), Vector2(float(world_width) - TANK_EDGE_MARGIN, 0.0)]
	_settle_tanks_on_terrain()
...
func _camera_target_x() -> float:
	...
	return clampf(focus_x - camera_world_width * 0.5, 0.0, maxf(0.0, float(world_width) - camera_world_width))
...
func _draw_distant_mountains() -> void:
	...
	for i: int in range(9):
		points.append(Vector2(float(i) * 150.0 - fmod(offset, 150.0) - 60.0, 295.0 + 48.0 * sin(float(i) * 1.7)))
...
func _draw_solid_mask() -> void:
	...
	for col: int in range(start_col, end_col + 1):
		...
		while row < terrain_rows:
			while row < terrain_rows and solid[col][row] == 0:
				row += 1
			if row >= terrain_rows:
				break
			var start_row: int = row
			while row < terrain_rows and solid[col][row] == 1:
				row += 1
			var end_row: int = row
			var screen_pos: Vector2 = _world_to_screen(Vector2(float(col * TERRAIN_STEP), float(start_row * TERRAIN_STEP)))
			var rect_size: Vector2 = Vector2(float(TERRAIN_STEP) * CAMERA_SCALE + 1.0, float((end_row - start_row) * TERRAIN_STEP) * CAMERA_SCALE + 1.0)
			draw_rect(Rect2(screen_pos, rect_size), Color(0.13, 0.24, 0.12), true)
...
func _draw_lake() -> void:
	...
	var water_height: float = 95.0 * CAMERA_SCALE
	draw_rect(Rect2(Vector2(left.x, left.y), Vector2(right.x - left.x, water_height)), Color(0.04, 0.24, 0.46, 0.78), true)
...
func _draw_exposed_edges() -> void:
	...
	var end_world_x: float = camera_x + VIEW_SIZE.x / CAMERA_SCALE
	var end_col: int = clampi(int(ceil(end_world_x / TERRAIN_STEP)) + 2, 0, terrain_cols - 1)
	var edge_color: Color = Color(0.30, 0.88, 0.38)
	var shadow_color: Color = Color(0.12, 0.32, 0.14)
	for col: int in range(start_col, end_col + 1):
		for row: int in range(0, terrain_rows):
			if solid[col][row] == 0:
				continue
			...
			if not _is_solid_cell(col, row - 1):
				draw_line(_world_to_screen(Vector2(x0, y0)), _world_to_screen(Vector2(x1, y0)), edge_color, EDGE_WIDTH)
			if not _is_solid_cell(col, row + 1):
				draw_line(_world_to_screen(Vector2(x0, y1)), _world_to_screen(Vector2(x1, y1)), shadow_color, EDGE_WIDTH * 0.75)
			if not _is_solid_cell(col - 1, row):
				draw_line(_world_to_screen(Vector2(x0, y0)), _world_to_screen(Vector2(x0, y1)), edge_color, EDGE_WIDTH)
			if not _is_solid_cell(col + 1, row):
				draw_line(_world_to_screen(Vector2(x1, y0)), _world_to_screen(Vector2(x1, y1)), edge_color, EDGE_WIDTH)
