...
func _make_surface_profile() -> PackedFloat32Array:
	...
	var control_spacing: float = rng.randf_range(70.0, 140.0)
	...
	for i: int in range(control_count):
		...
		var y: float = clampf(previous_y + rng.randf_range(-88.0, 88.0), TERRAIN_MIN_Y, TERRAIN_MAX_Y)
	...
	for col: int in range(terrain_cols):
		...
		if x > 230.0 and x < float(world_width) - 230.0:
			y += 6.0 * sin(x * 0.025 + rng.randf_range(-0.05, 0.05))
...
func _find_tank_surface_y(x: float) -> float:
	...
	for offset: float in sample_offsets:
		best_y = minf(best_y, _find_surface_y(clampf(x + offset, 0.0, float(world_width))))
	return best_y

func _settle_tanks_on_terrain() -> void:
	for player: int in range(2):
		tank_positions[player].y = _find_tank_surface_y(tank_positions[player].x) - TANK_RADIUS
...
func _update_tank_movement(delta: float) -> void:
	...
	tank_positions[current_player].y = _find_tank_surface_y(new_x) - TANK_RADIUS
...
func _draw_exposed_edges() -> void:
	...
	var end_world_x: float = camera_x + VIEW_SIZE.x / CAMERA_SCALE
	var end_col: int = clampi(int(ceil(end_world_x / TERRAIN_STEP)) + 2, 0, terrain_cols - 1)
	var edge_color: Color = Color(0.28, 0.82, 0.35)
	var edge_width: float = 1.6
	for col: int in range(start_col, end_col + 1):
		for row: int in range(0, terrain_rows):
			if solid[col][row] == 0:
				continue
			...
			if not _is_solid_cell(col, row + 1):
				draw_line(_world_to_screen(Vector2(x0, y1)), _world_to_screen(Vector2(x1, y1)), edge_color.darkened(0.25), edge_width)
