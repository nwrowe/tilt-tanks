...
# Temporary active weapon bridge while MainGame.gd is being flattened.
# Most newer behavior is already overridden in scripts/core/MainGame.gd. This
# layer now keeps only the declarations and fallback/runtime hooks still needed
# by the active chain, and delegates weapon/projectile/effect data to the helper
