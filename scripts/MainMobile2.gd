...
const TERRAIN_MAX_Y: float = 510.0
...
func _generate_lake_basin() -> void:
	...
	for i: int in range(terrain_points.size()):
		...
		var dist: float = absf(p.x - lake_center)
		if dist <= lake_width * 0.65:
			var nx: float = dist / (lake_width * 0.65)
			var basin_depth: float = 28.0 + 34.0 * (1.0 - nx * nx)
			var desired_y: float = lake_y + basin_depth
			p.y = maxf(p.y, desired_y)
			terrain_points[i] = p
...
func _base_surface_y_at_x(x: float) -> float:
...
func _ground_y_at_x(x: float) -> float:
	var y: float = _base_surface_y_at_x(x)
	var changed: bool = true
	var passes: int = 0
	while changed and passes < 8:
		changed = false
		passes += 1
		for hole: Vector3 in tunnel_holes:
			var dx: float = x - hole.x
			if absf(dx) <= hole.z:
				var dy: float = sqrt(maxf(0.0, hole.z * hole.z - dx * dx))
				var top_y: float = hole.y - dy
				var bottom_y: float = hole.y + dy
				if y >= top_y and y <= bottom_y:
					y = bottom_y + 1.0
					changed = true
	return y

func _is_in_tunnel(pos: Vector2) -> bool:
	for hole: Vector3 in tunnel_holes:
		if pos.distance_to(Vector2(hole.x, hole.y)) <= hole.z:
			return true
	return false
...
func _is_in_lake(pos: Vector2) -> bool:
	...
	if pos.x < lake_x or pos.x > lake_x + lake_width:
		return false
	return pos.y >= lake_y and pos.y <= _base_surface_y_at_x(pos.x) + 3.0

func _is_solid_terrain_at(pos: Vector2) -> bool:
	if _is_in_tunnel(pos) or _is_in_lake(pos):
		return false
...
func _apply_crater(pos: Vector2) -> void:
	var opened_surface: bool = false
	for i: int in range(terrain_points.size()):
		var point: Vector2 = terrain_points[i]
		var dx: float = point.x - pos.x
		if absf(dx) <= DAMAGE_RADIUS:
			var dy: float = sqrt(maxf(0.0, DAMAGE_RADIUS * DAMAGE_RADIUS - dx * dx))
			var top_y: float = pos.y - dy
			var bottom_y: float = pos.y + dy
			if point.y >= top_y and point.y <= bottom_y:
				point.y = clampf(bottom_y + 1.0, TERRAIN_MIN_Y, VIEW_SIZE.y + 90.0)
				terrain_points[i] = point
				opened_surface = true
	if not opened_surface:
		tunnel_holes.append(Vector3(pos.x, pos.y, DAMAGE_RADIUS))
		while tunnel_holes.size() > MAX_TUNNELS:
			tunnel_holes.pop_front()
	_refresh_terrain_line()
...
func _draw_lake() -> void:
	if not lake_enabled:
		return
	var water_poly: PackedVector2Array = PackedVector2Array()
	water_poly.append(_world_to_screen(Vector2(lake_x, lake_y)))
	water_poly.append(_world_to_screen(Vector2(lake_x + lake_width, lake_y)))
	var samples: int = int(maxf(6.0, lake_width / TERRAIN_STEP))
	for j: int in range(samples, -1, -1):
		var x: float = lake_x + lake_width * float(j) / float(samples)
		var bottom_y: float = maxf(lake_y + 4.0, _base_surface_y_at_x(x) - 2.0)
		water_poly.append(_world_to_screen(Vector2(x, bottom_y)))
	if water_poly.size() >= 3:
		draw_colored_polygon(water_poly, Color(0.04, 0.24, 0.46, 0.84))
	...
	draw_line(left, right, Color(0.18, 0.62, 0.95, 0.95), 3.0)

func _draw_tunnel_holes() -> void:
	for hole: Vector3 in tunnel_holes:
		var center: Vector2 = _world_to_screen(Vector2(hole.x, hole.y))
		var radius: float = hole.z * CAMERA_SCALE
		draw_circle(center, radius, Color(0.055, 0.045, 0.035))

func _draw_terrain_outline() -> void:
	var visible_points: PackedVector2Array = PackedVector2Array()
	for point: Vector2 in terrain_points:
		var sp: Vector2 = _world_to_screen(Vector2(point.x, _ground_y_at_x(point.x)))
		if sp.x >= -25.0 and sp.x <= VIEW_SIZE.x + 25.0:
