...
# -----------------------------------------------------------------------------
# Terrain mask
# solid[col][row] = 1 means dirt exists at that mask cell.
# This supports caves/tunnels/overhangs because each x can have many solid spans.
# -----------------------------------------------------------------------------
...
func _carve_lake_basin() -> void:
	...
	var water_bottom: float = lake_y + rng.randf_range(28.0, 55.0)
	for col: int in range(terrain_cols):
		var x: float = float(col * TERRAIN_STEP)
...
func _find_surface_y(x: float, preferred_y: float = 0.0) -> float:
	var col: int = clampi(int(round(x / TERRAIN_STEP)), 0, terrain_cols - 1)
	var start_row: int = 0
	if preferred_y > 0.0:
		start_row = clampi(int(floor((preferred_y + TANK_RADIUS) / TERRAIN_STEP)), 0, terrain_rows - 1)
	for row: int in range(start_row, terrain_rows):
		if solid[col][row] == 1:
			return float(row * TERRAIN_STEP)
	return float(WORLD_HEIGHT - TERRAIN_STEP)

func _rebuild_surface_points() -> void:
	surface_points.clear()
	for col: int in range(terrain_cols):
		var x: float = float(col * TERRAIN_STEP)
		var y: float = _find_surface_y(x)
		surface_points.append(Vector2(x, y))

func _settle_tanks_on_terrain() -> void:
	for player: int in range(2):
		var y: float = _find_surface_y(tank_positions[player].x, tank_positions[player].y)
		tank_positions[player].y = y - TANK_RADIUS
...
func _update_tank_movement(delta: float) -> void:
	...
	var new_y: float = _find_surface_y(new_x, tank_positions[current_player].y) - TANK_RADIUS
	if new_y < float(WORLD_HEIGHT - MAX_FALL_PIXELS):
		tank_positions[current_player].y = new_y
	else:
		tank_positions[current_player].y = new_y
	...
	for col: int in range(start_col, end_col + 1):
		...
		while row < terrain_rows:
			...
			while row < terrain_rows and solid[col][row] == 1:
				row += 1
			var end_row: int = row
			var world_pos: Vector2 = Vector2(float(col * TERRAIN_STEP), float(start_row * TERRAIN_STEP))
...
func _draw_lake() -> void:
	...
	var rect: Rect2 = Rect2(Vector2(left.x, left.y), Vector2(right.x - left.x, water_height))
	draw_rect(rect, Color(0.04, 0.24, 0.46, 0.78), true)
	draw_line(Vector2(left.x, left.y), Vector2(right.x, right.y), Color(0.18, 0.62, 0.95, 0.95), 3.0)

func _draw_surface_outline() -> void:
